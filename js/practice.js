...
//  v3 — timer expiry shows explanation, No Timer toggle
// =========================================================
...
const noTimerBtn       = document.getElementById('notimer-btn');
...
// ── No Timer toggle ───────────────────────────────────────
noTimerBtn.addEventListener('click', () => {
  noTimer = !noTimer;
  noTimerBtn.dataset.active = noTimer ? 'true' : 'false';
  noTimerBtn.classList.toggle('notimer-active', noTimer);

  if (noTimer) {
    // Hide the timer row completely
    timerRow.style.display = 'none';
    clearInterval(timerInterval);
  } else {
    // Show it again and restart timer for current question
    timerRow.style.display = 'flex';
    if (!frozen) startTimer();
  }
});
...
function startTimer() {
  ...
  // If No Timer mode is on, just track elapsed time invisibly (for stats)
  // but don't show or run down the bar
  if (noTimer) {
    timerRow.style.display = 'none';
    return;
  }
  ...
  timerInterval = setInterval(() => {
    ...
    timerBar.style.background = ratio > 0.5 ? 'var(--accent)'
                              : ratio > 0.25 ? '#f0a030'
                              : 'var(--wrong)';

    // ── Time's up — show explanation automatically ─────────
    if (ratio === 0) {
      clearInterval(timerInterval);
      if (!frozen) {
        frozen = true;
        // Flash the bar red briefly before showing explanation
        timerBar.style.width      = '100%';
        timerBar.style.background = 'var(--wrong)';
        timerBar.style.transition = 'none';
        // Count as wrong for stats
        totalWrong++;
        saveToStorage(false);
        updateStats();
        setTimeout(() => {
...
function checkFractionAnswer(type) {
  ...
  if (wRaw !== '' && nRaw === '' && dRaw === '') {
    return Math.abs(parseFloat(wRaw) - currentProblem.answer) < 0.001;
  }
  if (nRaw === '' || dRaw === '') return false;
  const den = parseFloat(dRaw);
  if (den === 0) return false;
  const value = (wRaw !== '' ? parseFloat(wRaw) : 0) + parseFloat(nRaw) / den;
...
// ── Explanation ───────────────────────────────────────────
function showExplanation(titleOverride) {
  frozen = true;
  stopTimer();
  disableInputs();
  ...
  explanationTitle.textContent = titleOverride
    ? `${titleOverride} The answer is ${ansStr}.`
    : `The answer is ${ansStr}.`;
